...
Midori for Android uses a few libraries for crash and exception reporting. This kind of reporting gives Mozilla invaluable insight as to why Midori for Android crashes or incorrectly behaves. It is one of the key methods we use to improve the product in terms of stability.

This page documents the types of crash reporting, how the various parts interact, and what kind of data is sent back to Midori Project.
...
In [HomeActivity](https://github.com/goastian/midori-android/blob/main/app/src/main/java/org/mozilla/fenix/HomeActivity.kt) when `onDestinationChanged` occurs, the destination fragment's name and and whether it is a custom tab is added to the breadcrumbs.
...
The server is hosted and maintained by Midori Project. There are no third-parties involved, all crash reports are sent directly from Midori for Android to the Sentry server hosted by Midori Project.

On the client side Sentry is invisible. There are no parts to interact with. It reports crashes and fatal errors back to Midori Project in the background.

On the server side there is a dashboard that the Midori for Android team uses to look at incoming crash reports. The dashboard lets us inspect the crash report in detail and for example see where in the application the crash happened, what version of the application was used and what version of Android OS was active. Below is an overview of all the attributes that are part of a crash report.
...
Sentry collects basic information about the Midori for Android app.

```
    "app":{
        "app_identifier":"org.midorinext.android",
        "app_name":"Midori Preview",
...
Every crash report contains a *stack trace*, which shows what functions in the Midori for Android code led to this crash. It includes names of Android framework functions and Midori for Android functions. Here's an excerpt of three lines from the stack trace:
...
Example of a Midori for Android generated message:
```
