...
/**
 * Midori implementation of [FxAEntryPoint].
 */
enum class MidoriFxAEntryPoint(override val entryName: String) : FxAEntryPoint, Parcelable {
    /**
     ...
     */
    companion object CREATOR : Parcelable.Creator<MidoriFxAEntryPoint> {
        override fun createFromParcel(parcel: Parcel): MidoriFxAEntryPoint {
            val parcelEntryName = parcel.readString() ?: Unknown
            return MidoriFxAEntryPoint.values().first { it.entryName == parcelEntryName }
        }

        override fun newArray(size: Int): Array<MidoriFxAEntryPoint?> {
