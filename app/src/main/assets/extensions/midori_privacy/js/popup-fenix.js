...
const renderMidoriActivity = function() {
    const pageCounts = popupData.pageCounts;
    const blocked = pageCounts?.blocked || {};
    const allowed = pageCounts?.allowed || {};
    const blockedTotal = blocked.any || 0;
    const requestTotal = blockedTotal + (allowed.any || 0);
    ...
    dom.text('#midoriTechnicalBlockedValue', formatNumber(blockedTotal));
    dom.text(
        '#midoriPopupBlocked',
        formatNumber(popupData.popupBlockedCount || 0)
    );
    qs$('#midoriActivity').style.setProperty(
        '--blocked-angle',
    ...
    dom.text(
        '#midoriPageCleanupStatus',
        i18n$(pageCleanupActive
            ? 'midoriStatusActive'
            : 'midoriStatusPaused')
    );
    dom.cl.toggle(
    ...
    for ( const [ name, value ] of Object.entries(breakdown) ) {
        ...
        dom.text(row.querySelector('strong'), formatNumber(value));
        dom.text(
            row.querySelector('small'),
            `${breakdownTotal === 0
                ? 0
                : Math.round(value * 100 / breakdownTotal)}%`
        );
