...
µb.loadUserSettings = async function() {
    ...
    const results = await Promise.all([
        ...
        vAPI.storage.get('midoriBadgeMigration'),
    ]);

    const usUser = results[0] instanceof Object && results[0] ||
                   Object.assign(usDefault);

    // Midori Privacy previously shipped a popup whose badge preference could
    // remain disabled across upgrades. Re-enable it once, while still allowing
    // managed policy below (and the user afterwards) to override the value.
    if ( results[2]?.midoriBadgeMigration !== 1 ) {
        usUser.showIconBadge = true;
        vAPI.storage.set({ midoriBadgeMigration: 1, showIconBadge: true });
    }
