...
import org.midorinext.android.components.MidoriSnackbar.Companion.LENGTH_SHORT
import org.midorinext.android.helpers.MockkRetryTestRule

class MidoriSnackbarDelegateTest {

    @MockK private lateinit var view: View
    @MockK(relaxed = true) private lateinit var snackbar: MidoriSnackbar
    private lateinit var delegate: MidoriSnackbarDelegate
    ...
    fun setup() {
        ...
        delegate = MidoriSnackbarDelegate(view)
        every {
            MidoriSnackbar.make(view, LENGTH_SHORT, isDisplayedWithBrowserToolbar = true)
        } returns snackbar
        every { snackbar.setText(any()) } returns snackbar
        every { snackbar.setAction(any(), any()) } returns snackbar
        every { view.context.getString(R.string.app_name) } returns "Midori"
    ...
    fun `show with no listener nor action`() {
        ...
        verify { snackbar.setText("Midori") }
