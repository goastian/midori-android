...
import org.midorinext.android.home.recentvisits.RecentlyVisitedItemInternal.HistoryGroupInternal
import org.midorinext.android.home.recentvisits.RecentlyVisitedItemInternal.HistoryHighlightInternal
...
class RecentVisitsFeatureTest {
    ...
    fun `GIVEN no recent visits WHEN feature starts THEN fetch history metadata and highlights then notify store`() =
        runTestOnMain {
            ...
            val highlightEntry = HistoryHighlight(1.0, 1, "https://astian.org", "midori", null)
            val recentHistoryHighlight = RecentHistoryHighlight("midori", "https://astian.org")
    ...
    fun `GIVEN groups containing history metadata items with the same url WHEN they are added to store THEN entries are deduped`() =
        runTestOnMain {
            ...
            val historyEntry2 = HistoryMetadata(
                key = HistoryMetadataKey("http://astian.org", "mozilla", null),
                title = "midori",
                createdAt = System.currentTimeMillis(),
                updatedAt = 2,
                totalViewTime = 20,
                documentType = DocumentType.Regular,
                previewImageUrl = "http://astian.org/image1"
            )
    ...
    fun `GIVEN different groups containing history metadata items with the same url WHEN they are added to store THEN entries are not deduped`() =
        runTestOnMain {
            ...
            val historyEntry3 = HistoryMetadata(
                key = HistoryMetadataKey("http://www.mozilla.com", "midori", null),
                title = "mozilla",
                createdAt = now,
                updatedAt = now + 1,
                totalViewTime = 30,
