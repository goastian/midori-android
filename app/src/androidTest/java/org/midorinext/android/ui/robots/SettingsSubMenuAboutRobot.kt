...
class SettingsSubMenuAboutRobot {
    ...
    fun verifyAboutMidoriPreview(rule: ComposeTestRule) = assertMidoriPreviewPage(rule)
...
private fun assertMidoriPreviewPage(rule: ComposeTestRule) {
    assertAboutToolbar()
...
private fun assertProductCompany(rule: ComposeTestRule) {
    rule.onNodeWithText("$appName is made by Astian, Inc.")
        .assertIsDisplayed()
}
...
private fun assertLibrariesUsed(rule: ComposeTestRule) {
    ...
    // TODO: [Midori] fix this, "MPL" substring should be present in license URL
